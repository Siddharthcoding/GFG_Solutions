...
class Solution {
  public:
    int solve(vector<int>& prices, int k, bool canBuy, int idx) {
        if(idx == prices.size() || k == 0)
        return 0;
        
        int take = 0, skip = 0;
        if(canBuy) {
            take = -prices[idx] + solve(prices, k, false, idx+1);
            skip = solve(prices, k, true, idx+1);
        } else {
            take = prices[idx] + solve(prices, k-1, true, idx+1);
            skip = solve(prices, k, false, idx+1);
        }
    ...
    int maxProfit(vector<int>& prices, int k) {
        // code here
        bool canBuy = true;
        return solve(prices, k, canBuy, 0);
    }
};
```

# Optimized Approach(Using recursive DP)
